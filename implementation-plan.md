...
- [x] 实现文件夹选择对话框（IPC）
- [x] 扫描并筛选 CSV 文件
- [x] 自动检测文件编码（使用 `chardet`）
- [x] 显示文件列表

##### 2.1.2 CSV 解析
- [x] 集成 `papaparse` 解析器
- [x] 处理 RFC 4180 标准（引号、换行符、逗号）
- [x] 提取表头和数据行
- [x] 缓存第一列为 Key 值
...
##### 2.2.1 虚拟滚动表格
- [x] 集成 `react-window` 的 `FixedSizeGrid`
- [x] 实现单元格渲染组件
- [x] 优化性能（`React.memo`）
- [x] 测试 50000 行数据渲染性能
...
##### 2.2.4 复制粘贴
- [x] 复制选中单元格（`Ctrl/Cmd + C`）
- [x] 粘贴到目标位置（`Ctrl/Cmd + V`）
- [x] 支持跨行粘贴（Tab 分隔）
...
##### 3.1.1 基础集成
- [x] 创建 CodeMirror 6 编辑器组件
- [x] 配置基础扩展（`basicSetup`）
- [x] 双向绑定单元格内容

##### 3.1.2 TMP 标签解析
- [x] 编写标签正则表达式（`<color>`, `<size>`, `<b>`, `<i>`）
- [x] 实现标签解析器
- [x] 处理嵌套标签
...
##### 3.5.1 历史栈实现
- [x] 创建 `HistoryStore` (基于 Zustand)
- [x] 实现 `HistoryEntry` 类型定义 (Command Pattern)
- [x] 实现全局 `pushEntry`, `undo`, `redo` actions
- [x] 限制历史栈大小（100 条）

##### 3.5.2 操作记录适配 (Commands)
- [x] 重构 `ProjectStore` 以支持 Command Pattern (Integrated via Store Actions)
- [x] 实现 `CellEditCommand` (单元格编辑)
- [x] 实现 `PasteCommand` (批量粘贴 - via batchUpdateCells)
- [x] 实现 `FillCommand` (填充柄 - via batchUpdateCells)
- [x] 实现 `RowInsertCommand` / `RowDeleteCommand`
- [x] 实现 `ColInsertCommand` / `ColDeleteCommand`

##### 3.5.3 快捷键与UI
- [x] 绑定 `Ctrl/Cmd + Z` (Undo)
- [x] 绑定 `Ctrl/Cmd + Shift + Z` (Redo)
- [ ] 撤销后自动聚焦变更区域 (Partially implemented via Selection State)
...
- [ ] 配置 macOS 打包（DMG）
- [ ] 设置应用图标
- [x] 配置自动更新（GitHub Releases + electron-updater）
