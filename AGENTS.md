...
1. 日志输出采用摘要模式  
- 命令执行后仅反馈关键结论与必要行（错误、警告、结果摘要）。
- 不回传完整长日志，除非用户明确要求“贴完整输出”。

2. 构建按需执行  
- 日常改动优先执行：`npx tsc --noEmit` + `npm run test:unit -- --runInBand`。
- 仅在发版、打包验证或用户明确要求时执行 `npm run build` / `npm run build:win`。

3. 禁止扫描构建产物目录  
- 搜索、diff、排查时默认排除：`dist-renderer/`、`dist-release/`、`dist-release-2/`、`dist-electron/`。
- 除非用户明确要求，不读取这些目录内的大文件与二进制产物。
