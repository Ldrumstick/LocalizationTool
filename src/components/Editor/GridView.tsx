...
const GridView: React.FC<GridViewProps> = ({ headers, rows }) => {
  ...
  const Cell = useCallback(({ columnIndex, rowIndex, style }: any) => {
    ...
    return (
      <div
        style={style}
        className={`grid-cell ${isSelected ? 'selected' : ''} ${isInSelectedRange ? 'selected-range' : ''} ${isInvalidKey ? 'invalid-key' : ''} ${
          isCurrentMatch ? 'current-match' : isOtherMatch ? 'search-match' : ''
      ...
      >
        {isCurrentEditing ? (
          editingLocation === 'cell' ? (
            <InlineEditor row={rowIndex} col={columnIndex} value={content} onNavigate={navigateCell} />
          ) : (
            // 当在编辑栏编辑时，显示 tempValue，保持选中样式
            <div className="cell-content-preview" style={{ whiteSpace: 'pre-wrap' }}>{tempValue}</div>
          )
        ) : (
          content
        )}
  ...
  }, [rows, selectedCell, selectedRange, setSelectedCell, searchResults, currentResultIndex, selectedFileId, isEditing, editingCell, enterEditMode, navigateCell, tempValue, editingLocation, fillState]);
