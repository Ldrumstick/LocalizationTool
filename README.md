...
- 🚀 **高性能**: 主编辑表格基于 Glide Data Grid，流畅处理 50000+ 条目
...
#### 表格编辑器
- 选中单元格后可直接输入进入编辑；双击单元格打开 Glide 原生浮层编辑器
- `Enter` 键跳转下一行，`Tab` 键移动到右侧
- 支持 Glide 原生复制粘贴、批量插入/删除与尾行新增
- 支持从 Excel / TSV 复制多格内容后按 Glide 原生规则粘贴到当前表格范围
- 填充柄拖动：普通内容走 Glide 原生填充；数字或带数字后缀的序列会递增填充
- 表头菜单支持重命名列
- Toggle 列只在点击单元格中央开关控件时切换值，点击周围空白仅选中单元格
- 非法 Key 自动标红
